...
export default (vue, form) => {
    ...
    return Vue.extend({
        ...
        watch: {
            'user.option.filter.search': function(val){
                this.lazy(()=>{
                    this.user.all(this);
                })
            },
            'user.option.collapse.filter': function(n){
        ...
        created(){
            this.user
                ...
                .pushAction('url_prefix', ()=>"user_")
                .setCollapse(this, {
                    filter: this.user.getStore('filter', this.user.getCollapse('filter', false), e => {
